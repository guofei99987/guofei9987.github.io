...
在推导中发现，如果以梯度$\delta$为核心，算法看起来会更优美，并可以写的更有一般性（实际还是等价的）  
...
- 标量形式：$\delta_j^l=\dfrac{\partial C}{\partial u_j^l}=\sum_k\dfrac{\partial C}{\partial u_j^{l+1}}\dfrac{\partial u_k^{l+1}}{\partial v_j^l}\dfrac{\partial v_j^l}{\partial u_j^l}=\sum_k\sigma_k^{l+1}w_{jk}^l \sigma'(u_j^l)$
### BP3
